...
* `jirametrics info` now finds an issue even when your configuration told the report to ignore it, and says whether the issue was used or ignored. Previously an issue excluded by something like `ignore_types` was reported as not found at all, which is misleading when it is sitting right there in the downloaded data. It also records which projects it searched and which directory each one read its issues from. If it cannot find an issue you know was downloaded, the log will show you where it actually looked, which is usually enough to spot that the config switched target directories partway through. Changes to the target directory are logged too. The terminal output is unchanged apart from a pointer to the log.
