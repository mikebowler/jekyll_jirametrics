...
* New `jirametrics verify` command. It checks that JiraMetrics can authenticate to each configured Jira instance without downloading any data, and exits with a non-zero status if any connection fails. Handy as a first step to confirm your credentials before the slow download. See [command line usage]({% link usage.md %}).
* New `jirametrics boards` command. On its own it lists every board you can access (id, name and type, sorted by name, with an optional `--name` filter). Given a board id it shows that board's columns and the statuses in each, with their categories, to help you choose cycle time start and stop points. For a team-managed board it works out whether the board uses sprints, and for any sprint board it also suggests `first_time_added_to_active_sprint` as a start. See [command line usage]({% link usage.md %}).
