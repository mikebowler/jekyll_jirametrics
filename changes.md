...
* GitHub pull request downloads no longer throw away good data when a repo fails. Previously, if fetching PRs from any one configured `github_repos` entry failed (the repo was renamed or made private, or your `gh` CLI lost access), the entire download aborted; and because each run rewrites the `*_github_prs.json` file, a fetch that returned nothing could quietly replace previously downloaded PR data with an empty list. Now a failing repo logs a warning and JiraMetrics leaves the existing PR file untouched rather than overwriting it with an incomplete set. A genuinely empty but successful fetch is still saved as before. This matters because charts that use PR data (such as a custom lead-time chart) silently fall back to different calculations when the PRs go missing, so two machines could produce very different numbers from the same Jira data.
